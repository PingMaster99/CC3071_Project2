    ...
    def handle_keyword(self, keyword_list, data):
        ...
        if keyword == 'COMPILER':
            ...
            for element in data:
                line = element.pop()
                if '=' not in element:
                    print_error(line, "'=' sign not present in CHARACTER definition")
            ...
            for element in data:
                line = element.pop()
                if '=' not in element:
                    print_error(line, "'=' sign not present in KEYWORD definition")
                    # TODO Equal sign in another position
                elif '.' not in element[-1]:
                    print_error(line, "'.' not present at line end")
            ...
            automaton.match_tokens(read_file_characters())
